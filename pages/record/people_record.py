...
async def build_people_record_view(page: ft.Page) -> ft.View:
    """人员进出记录列表"""

    async def _load(page_no: int, page_size: int, filters: dict) -> dict:
        return await rs.get_people_record_list(
        ...
        return ft.Container(
            ...
            content=ft.Column(
                ...
                controls=[
                    ...
                    ft.Row([
                        ft.Text(f"姓名: {item.get('name', '-')}", size=13, color=ft.colors.GREY_600),
                        ft.Text(f"进出区分: {item.get('accessAction', '-')}", size=13, color=ft.colors.GREY_600),
                    ]),
                    ft.Row([
                        ft.Text(f"门: {item.get('gateName', '-')}", size=13, color=ft.colors.GREY_600),
                        ft.Text(f"通道: {item.get('aisleName', '-')}", size=13, color=ft.colors.GREY_600),
                    ]),
                ],
            ),
        )
