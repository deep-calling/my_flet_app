...
async def build_brush_view(page: ft.Page) -> ft.View:
    """在线刷题列表页"""

    current_tab = [0]  # 0=刷题列表, 1=刷题结果
    current_page_no = [1]
    page_size = 10
    ...
    async def _load_data(reset: bool = False):
        ...
        try:
            ...
            if current_tab[0] == 0:
                result = await ts.get_brush_list(params)
            else:
                result = await ts.get_brush_complete_list(params)
                    ...
                    async def _click(e):
                        if current_tab[0] == 0:
                            ...
                            await page.go_async(
                                ...
                                f"&examId={data.get('examId', '')}&type=1"
                            )
                        else:
                            # 查看刷题结果
                            await page.go_async(
                                f"/train/details?examId={data.get('examId', '')}"
                                f"&type=1&resultId={data.get('resultId', '')}"
                            )
                    return _click
        ...
        return ft.Container(
            content=ft.Column(
                controls=[
                    ...
                    ft.Text(
                        f"答题方式：{item.get('answerMethod_dictText', '-')}",
                        size=13, color=ft.colors.GREY_600,
                    ),
                    ft.Text(
                        f"出题人：{item.get('person_dictText', '-')}",
    ...
    tabs = ft.Tabs(
        ...
        tabs=[ft.Tab(text="刷题列表"), ft.Tab(text="刷题结果")],
    ...
    view = ft.View(
        route="/train/brush",
        appbar=ft.AppBar(title=ft.Text("在线刷题"), bgcolor=ft.colors.WHITE),
        controls=[body],
        padding=0,
