"""监测报警 — 列表页"""

from __future__ import annotations

import flet as ft

from components.list_page import build_list_page
from services import alarm_service as als


async def build_indicators_alarm_view(page: ft.Page, today: str = "") -> ft.View:
    """监测报警列表，支持待处理/所有切换，today 参数高亮今日"""

    current_tab = [0]  # 0=待处理, 1=所有

    async def _load(page_no: int, page_size: int, filters: dict) -> dict:
        params: dict = {"pageNo": page_no, "pageSize": page_size}
        if current_tab[0] == 0:
            params["alarmStatus"] = "0"
        if today:
            params["today"] = today
        return await als.get_sensor_alarm_list(params)

    def _build_item(item: dict) -> ft.Control:
        return ft.Container(
            bgcolor=ft.colors.WHITE,
            border_radius=8,
            padding=ft.padding.all(14),
            margin=ft.margin.only(left=12, right=12, top=8),
            content=ft.Column(
                spacing=6,
                controls=[
                    ft.Text(f"点位名称: {item.get('pointName', '-')}", size=13, color=ft.colors.GREY_700),
                    ft.Text(f"点位号: {item.get('pointCode', '-')}", size=13, color=ft.colors.GREY_600),
                    ft.Text(f"报警类型: {item.get('alarmType_dictText', '-')}", size=13, color=ft.colors.GREY_600),
                    ft.Text(f"当前报警值: {item.get('currentValue', '-')}", size=13, color=ft.colors.GREY_600),
                    ft.Text(f"报警阀值: {item.get('alarmValue', '-')}", size=13, color=ft.colors.GREY_600),
                    ft.Text(f"报警时间: {item.get('alarmTime', '-')}", size=13, color=ft.colors.GREY_600),
                ],
            ),
        )

    async def _on_click(item: dict):
        page.go(f"/alarm/indicators_detail?alarmId={item['id']}")

    view = await build_list_page(
        page,
        title="监测报警",
        on_load_data=_load,
        build_item=_build_item,
        on_item_click=_on_click,
        show_search=False,
    )

    # 插入 Tab 栏
    tabs = ft.Tabs(
        selected_index=0,
        tabs=[ft.Tab(text="待处理"), ft.Tab(text="所有")],
        height=42,
    )

    async def _on_tab_change(e):
        current_tab[0] = tabs.selected_index
        page.go(page.route)

    tabs.on_change = _on_tab_change

    body_col = view.controls[0]
    body_col.controls.insert(0, ft.Container(
        content=tabs,
        bgcolor=ft.colors.WHITE,
    ))

    return view
